...
void GameModeCommand::execute(shared_ptr<CommandSender> source, byteArray commandData)
{
    ...
    shared_ptr<ServerPlayer> player =
        MinecraftServer::getInstance()->getPlayers()->getPlayer(uid);
    if (player == nullptr) return;

    GameType* newMode = GameType::byId(modeId);
    if (newMode == nullptr) return;

    player->setGameMode(newMode);
    player->resetLastActionTime();
}

shared_ptr<GameCommandPacket> GameModeCommand::preparePacket(
    shared_ptr<Player> player, int gameMode)
{
...
GameType* GameModeCommand::getModeForString(
    shared_ptr<CommandSender> source, const wstring& name)
{
